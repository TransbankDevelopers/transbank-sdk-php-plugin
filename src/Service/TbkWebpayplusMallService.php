...
final class TbkWebpayplusMallService extends BaseWebpayplusService {
    
    /**
     * @var \Transbank\Webpay\WebpayPlus\MallTransaction
     */
    private $webpayplusMallTransaction;
    ...
    public function __construct(ILogger $logger, IConfigRepository $configRepository, ApiServiceLogService $apiServiceLogService, ExecutionErrorLogService $executionErrorLogService, TransactionService $transactionService, $storeId = '0') {
        ...
        $config = $this->configRepository->getWebpayplusMallConfig()->getMallProductConfigByStoreId($storeId);
        $this->options = $this->createOptions($config);
        $this->config = $config;
        $this->webpayplusMallTransaction = new MallTransaction($this->options);
        $this->product = TbkConstants::TRANSACTION_WEBPAY_PLUS_MALL;
    }
    ...
    */
    ...
    public function sdkStatus($token){
        return $this->webpayplusMallTransaction->status($token);
    }

    public function sdkRefund(TransbankTransactionDto $tx, $amount){
        return $this->webpayplusMallTransaction->refund($tx->getToken(), $tx->getBuyOrder(), $tx->getChildCommerceCode(), $amount);
    }
    ...
    protected function createTransactionInTbk(TransbankTransactionDto $tx, $returnUrl){
        ...
        try {
            ...
            $response = $this->webpayplusMallTransaction->create($tx->getBuyOrder(), $tx->getSessionId(), $returnUrl,
            [[
                "amount" => $tx->getAmount(),
    ...
    public function createTransaction($orderId, $amount, $returnUrl){
        ...
        $tx->setChildBuyOrder('chi:'.$randomNumber.':'.$orderId);
        $tx->setAmount($amount);
        ...
        $tx = $this->transactionService->createForWebpayplusMall($tx, $eel);
    ...
    private function commitTransactionInTbk($buyOrder, $token, $tx)
    {
        ...
        try {
            ...
            $response = $this->webpayplusMallTransaction->commit($token);
            $asl->setResponse(json_encode($response));
            $this->afterExecutionTbkApi($asl);
            return $response;
        } catch (Exception $e) {
    ...
    public function commitTransaction($token)
    {
        ...
        if (!$commitResponse->isApproved()) {
            $errorMessage = 'El commit de la transacción ha sido rechazada en Transbank (código de respuesta: '.$commitResponse->getDetails()[0]->getResponseCode().')';
        ...
        $tx->setTransbankStatus($commitResponse->getDetails()[0]->getStatus());
