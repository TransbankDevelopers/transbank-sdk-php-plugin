...
class TbkValidationUtil {
    ...
    public static function validateCommerceCode($commerceCode, $label = "código de comercio"){
        if (!StringUtils::hasLength($commerceCode, 12)){
            return "El {$label} debe tener 12 digitos";
        }
        if (!strpos($commerceCode, '5970') === 0) {
            return "El {$label} debe comenzar con 5970";
        }
        return null;
    }

    public static function validateApikey($apikey){
        return StringUtils::isBlankOrNull($apikey) ? "El 'apikey' es nulo o vacio" : null;
    }

    public static function validateOrderStatusAfterPayment($status){
        return StringUtils::isBlankOrNull($status) ? "El 'estado de pago aceptado' es nulo o vacio" : null;
    }

    public static function throwExceptionIfError($error){
        if (StringUtils::isNotBlankOrNull($error)){
            throw new Exception($error);
        }
    }
    
    public static function validateProductConfigThrowException(ProductConfig $product){
        TbkValidationUtil::throwExceptionIfError(TbkValidationUtil::validateProductConfig($product));
    }

    public static function validateMallProductConfigThrowException(MallProductConfig $product){
        TbkValidationUtil::throwExceptionIfError(TbkValidationUtil::validateMallProductConfig($product));
    }

    public static function validateProductConfig(ProductConfig $product){
        $error = TbkValidationUtil::validateCommerceCode($product->getCommerceCode());
        if (StringUtils::isBlankOrNull($error)){
            return $error;
        }
        $error = TbkValidationUtil::validateApikey($product->getApikey());
        if (StringUtils::isBlankOrNull($error)){
            return $error;
        }
        $error = TbkValidationUtil::validateOrderStatusAfterPayment($product->getOrderStatusAfterPayment());
        if (StringUtils::isBlankOrNull($error)){
            return $error;
        }
        return null;
    ...
    public static function validateMallProductConfig(MallProductConfig $product){
        $error = TbkValidationUtil::validateProductConfig($product);
        if (StringUtils::isBlankOrNull($error)){
            return $error;
        }
        if (!$product->isMultiStore()){
            $error = TbkValidationUtil::validateCommerceCode($product->getChildCommerceCode(), "Código de comercio Mall");
            if (StringUtils::isBlankOrNull($error)){
                return $error;
            }
        }
    ...
    public static function isDateValidForApiOperation($date, $days){
        $temp = DateTime::createFromFormat('Y-m-d H:i:s', $date);
        if ($temp!=null && !$temp){
            $now = new DateTime();
            $interval = $now->diff($temp);
    ...
    public static function validateRefundDate($createdAt){
        if (!TbkValidationUtil::isDateValidForApiOperation($createdAt, 90)){
            return "El refund no esta permitido en transacciones con mas de 90 dias";
        }
        return null;
    }

    public static function validateStatusDate($createdAt){
        if (!TbkValidationUtil::isDateValidForApiOperation($createdAt, 7)){
            return "El status no esta permitido en transacciones con mas de 7 dias";
        }
        return null;
    }
    ...
    public static function checkAccessibleProductionUrl(){
        return TbkValidationUtil::checkAccessibleTbkUrl(
    ...
    public static function addError($errors, $error){
        if (!is_null($error)){
            $errors[] = $error;
        }
    }

    public static function errorsToMessage($errors){
